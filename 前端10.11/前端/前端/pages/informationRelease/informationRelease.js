...
({
  ...
  onPreviewImage(event){
    ...
    console.log(event.target.dataset.imgsrc.image)
    wx.previewImage({
      urls: [ds.imgsrc.image],
  ...
  onLoad: function (options) 
  {
    wx.request
    ({
      url: 'http://127.0.0.1:8083/user/InformationReleaseList',
      data:{},
      success:(res)=>
      {
          console.log('请求成功：',res.data);
          ...
          this.setData
          ({
            informationReleaseList: res.data.data
          })
      },
      fail:(err)=>
      {
